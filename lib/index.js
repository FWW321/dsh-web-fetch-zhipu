 ...
 * (`web-fetch-zhipu`) for hot edits. No session-event audit: the rc.6
 * persistence read path refuses unknown event types (closed
 * KNOWN_SESSION_EVENT_TYPES, no public ignorable write path), so custom
 * events poison the log.
 */
 ...
 * is normally double-encoded (content[0].text is a JSON string of another
 * JSON string of the `{title, url, content, metadata, external}` object),
 * but the reader's response shape is NOT stable: it intermittently returns
 * plain text instead (observed on cache hits / degraded conversions). Unwrap
 * string layers while they parse; when the result never reaches the object
 * shape, fall back to treating the joined text itself as the document body
 * instead of dropping the fetch. `url` is backfilled by the caller.
 */
function decodeReaderDocument(payload) {
	...
	if (current === null || typeof current !== "object" || typeof current.content !== "string") {
		if (typeof current === "string" && current.trim().length > 0) return { content: current };
		return null;
	}
	return current;
}
...
class ZhipuFetchProvider {
	...
	async #mcpFetch(request, apiKey, options, signal) {
		...
		// truncation flags our char cap. Body key is `content` (WebFetchBody
		// closed union in dsh-web): `text` would surface as undefined and the
		// tool output snapshot rejects it ("value is not lossless JSON").
		...
		return {
			...
			body: {
				...
				content: truncated ? document.content.slice(0, options.maxOutputChars) : document.content,
			},
			truncated,
		};
	}
...
function apply(ctx, config) {
	...
	ctx.web.registerFetchProvider(new ZhipuFetchProvider(() => resolveOptions(current())));
}
